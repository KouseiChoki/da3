"""
Depth Anything 3 Streaming Control for TouchDesigner

This script provides a parameter-driven interface to configure and control
the DA3 streaming server connection with automatic reconnection and backend management.
...
def onSetupParameters(scriptOp):
    ...
    page.appendStr('Port', label='Port')
    ...
    page.appendInt('Quality', label='JPEG Quality', default=75, min=50, max=95)

    # Input Settings
    page.appendInt('Frameskip', label='Frame Skip (N)', default=1, min=1, max=10)
    page.appendInt('Inputwidth', label='Input Width', default=640, min=128, max=1920)
    page.appendInt('Inputheight', label='Input Height', default=480, min=128, max=1080)
...
def onReceiveText(dat, message):
    ...
    try:
        ...
        if data.get('status') == 'success':
            # Get base64 encoded JPEG depth map
            depth_b64 = data['data']
            depth_bytes = base64.b64decode(depth_b64)

            # Write to temp file for Movie File In TOP to load
            temp_dir = parent().fetch('depth_temp_dir', None)
            if temp_dir is None:
                temp_dir = tempfile.gettempdir()
                parent().store('depth_temp_dir', temp_dir)

            temp_path = os.path.join(temp_dir, 'da3_depth_latest.jpg')
            with open(temp_path, 'wb') as f:
                f.write(depth_bytes)

            # Store path for Movie File In TOP
            parent().store('depth_file_path', temp_path)

            # Trigger Movie File In TOP to reload
            depth_movie = op('depth_display')
            if depth_movie:
                depth_movie.par.file = temp_path
                depth_movie.par.reload.pulse()
